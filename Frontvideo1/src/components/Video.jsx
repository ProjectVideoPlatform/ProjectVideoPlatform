...
const PaymentModal = ({ video, onClose, onSuccess }) => {
  ...
  React.useEffect(() => { createQRPayment(); }, []);

  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden">
        <div className="bg-gradient-to-r from-blue-600 to-purple-600 p-6 text-white relative">
          <button onClick={onClose} className="absolute top-4 right-4 text-white hover:bg-white hover:bg-opacity-20 rounded-full p-1 transition-colors">
            <X className="w-6 h-6" />
          </button>
          <div className="flex items-center space-x-3">
            <div className="bg-white bg-opacity-20 p-3 rounded-full"><QrCode className="w-6 h-6" /></div>
            <div>
          ...
          <div className="flex flex-col items-center">
            ...
            {qrImageUrl && paymentStatus === 'checking' && (
              <div className="w-full">
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
                  ...
                  <p className="text-blue-600 text-sm text-center mt-2">กรุณาสแกน QR Code ด้วยแอปธนาคารของคุณ</p>
                </div>
              </div>
            )}
            {paymentStatus === 'success' && (
              <div className="w-full py-8 flex flex-col items-center">
                ...
                <p className="text-gray-600 text-center">คุณสามารถรับชมวิดีโอได้แล้ว</p>
              </div>
            )}
            {paymentStatus === 'failed' && (
              <div className="w-full py-8 flex flex-col items-center">
                ...
                <p className="text-gray-600 text-center mb-4">กรุณาลองใหม่อีกครั้ง</p>
                <button onClick={createQRPayment} className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">ลองใหม่</button>
              </div>
            )}
          </div>
          {qrImageUrl && paymentStatus === 'checking' && (
...
const VideoCard = ({ video, onPlay, isLoading, isAdmin }) => {
  ...
  const canWatch = isAdmin || videoPurchased || video.canPlay || video.purchased;
  ...
  const formatPrice = (price) => price === 0 ? 'ฟรี' : `฿${price.toFixed(2)}`;
    ...
    if (video.uploadStatus === 'processing') {
      return (
        <div className="absolute top-2 left-2 bg-yellow-500 text-white text-xs px-2 py-1 rounded flex items-center">
          <Loader className="w-3 h-3 mr-1 animate-spin" />กำลังประมวลผล
        </div>
      );
    }
    if (video.uploadStatus === 'failed') {
      return (
        <div className="absolute top-2 left-2 bg-red-500 text-white text-xs px-2 py-1 rounded flex items-center">
          <AlertCircle className="w-3 h-3 mr-1" />ล้มเหลว
        </div>
      );
    }
    return null;
    ...
    <>
      <div className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
        ...
        <div className="p-4">
          ...
          {video.description && <p className="text-gray-600 text-sm mb-3 line-clamp-3">{video.description}</p>}
          ...
          <div className="flex items-center justify-between">
            <span className="font-bold text-lg text-green-600">{formatPrice(video.price)}</span>
            <div className="flex gap-2">
              {video.uploadStatus === 'completed' && canWatch ? (
                <button onClick={() => onPlay(video)} disabled={isLoading} className="flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50">
                  {isLoading ? <Loader className="w-4 h-4 animate-spin mr-2" /> : <Play className="w-4 h-4 mr-2" />}เล่น
                </button>
              ) : video.uploadStatus === 'completed' && !canWatch ? (
                <button onClick={() => setShowPaymentModal(true)} disabled={isLoading} className="flex items-center px-4 py-2 bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-lg hover:from-green-700 hover:to-emerald-700 transition-all shadow-md disabled:opacity-50">
                  {isLoading ? <Loader className="w-4 h-4 animate-spin mr-2" /> : <ShoppingCart className="w-4 h-4 mr-2" />}ซื้อ
                </button>
              ) : (
          ...
          {(videoPurchased || video.purchased) && !isAdmin && (
            <div className="mt-2 flex items-center text-green-600 text-sm">
              <CheckCircle className="w-4 h-4 mr-1" />เป็นเจ้าของแล้ว
              {video.purchaseInfo && (
                <span className="ml-2 text-gray-500">• ซื้อเมื่อ {new Date(video.purchaseInfo.purchaseDate).toLocaleDateString('th-TH')}</span>
              )}
            </div>
          )}
          {isAdmin && (
      ...
      {showPaymentModal && (
        <PaymentModal
          ...
          onSuccess={() => { setVideoPurchased(true); setShowPaymentModal(false); }}
        />
      )}
    </>
  );
...
const UploadModal = ({ isOpen, onClose, onUpload }) => {
  ...
  const handleClose = () => {
    if (!uploading) { onClose(); resetForm(); }
  };
  ...
  const stepConfig = {
    ...
    processing: { label: 'Processing video...', progress: 100 },
    failed: { label: '❌ Upload failed. Please try again.', progress: 100 },
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
      <div className="bg-white rounded-lg p-6 w-full max-w-md mx-4">
        <div className="flex justify-between items-center mb-4">
          ...
          <button onClick={handleClose} disabled={uploading} className="text-gray-500 hover:text-gray-700 disabled:opacity-50">
            <X className="w-5 h-5" />
          </button>
        </div>
        {activeStep && (
          <div className="mb-6">
            <div className="flex justify-between text-sm mb-2">
              <span className={currentStep === 'uploading' ? 'text-blue-600 font-medium' : 'text-gray-400'}>Uploading</span>
              <span className={currentStep === 'processing' ? 'text-blue-600 font-medium' : 'text-gray-400'}>Processing</span>
              <span className={currentStep === 'failed' ? 'text-red-600 font-medium' : 'text-gray-400'}>Done</span>
            </div>
            <div className="w-full bg-gray-200 rounded-full h-2">
              <div
                className={`h-2 rounded-full transition-all duration-500 ${currentStep === 'failed' ? 'bg-red-500' : 'bg-blue-600'}`}
                style={{ width: `${activeStep.progress}%` }}
              />
            </div>
            <p className={`text-sm mt-2 ${currentStep === 'failed' ? 'text-red-600' : 'text-gray-600'}`}>{activeStep.label}</p>
          </div>
        )}
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium mb-1">Video File *</label>
            <input type="file" accept="video/*" onChange={(e) => setFile(e.target.files[0])} disabled={uploading} className="w-full px-3 py-2 border rounded-lg disabled:bg-gray-100" />
            {file && <p className="text-xs text-gray-600 mt-1">{file.name} ({(file.size / 1024 / 1024).toFixed(1)} MB)</p>}
          </div>
          <div>
            <label className="block text-sm font-medium mb-1">Title *</label>
            <input type="text" value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })} disabled={uploading} className="w-full px-3 py-2 border rounded-lg disabled:bg-gray-100" />
          </div>
          <div>
            <label className="block text-sm font-medium mb-1">Description</label>
            <textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} disabled={uploading} className="w-full px-3 py-2 border rounded-lg h-24 resize-none disabled:bg-gray-100" />
          </div>
          <div>
            <label className="block text-sm font-medium mb-1">Price (฿)</label>
            <input type="number" min="0" step="0.01" value={formData.price} onChange={(e) => setFormData({ ...formData, price: parseFloat(e.target.value) || 0 })} disabled={uploading} className="w-full px-3 py-2 border rounded-lg disabled:bg-gray-100" />
          </div>
          <div>
            <label className="block text-sm font-medium mb-1">Tags (comma separated)</label>
            <input type="text" value={formData.tags} onChange={(e) => setFormData({ ...formData, tags: e.target.value })} disabled={uploading} placeholder="action, comedy, thriller" className="w-full px-3 py-2 border rounded-lg disabled:bg-gray-100" />
          </div>
          <div className="flex gap-2 pt-4">
            <button onClick={handleClose} disabled={uploading} className="flex-1 px-4 py-2 border rounded-lg hover:bg-gray-50 disabled:opacity-50">
              {uploading ? 'Please wait...' : 'Cancel'}
            </button>
            <button onClick={handleSubmit} disabled={uploading || !file || !formData.title} className="flex-1 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50">
...
const handleWsEvent = useCallback(async (data) => {
  ...
  const loadVideos = async (params = {}) => {
    ...
    try {
      const result = await api.getVideos({
        ...
        search: searchQuery, category: selectedCategory, ...params
      });
  ...
  const loadPurchasedVideos = async () => {
    setLoading(true);
    try {
      const result = await api.getPurchasedVideos({ page: currentPage, limit: 12 });
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center space-x-4">
              {isAdmin && (
                <button onClick={() => setShowUploadModal(true)} className="flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
                  <Upload className="w-4 h-4 mr-2" />Upload
                </button>
              )}
              <button onClick={GotoUserProfile} className="flex items-center px-4 py-2 border rounded-lg hover:bg-gray-50">
      ...
      <nav className="bg-white border-b">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex space-x-8">
            <button onClick={() => handleViewChange('all')} className={`py-4 px-2 border-b-2 font-medium text-sm ${currentView === 'all' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>
              All Videos
            </button>
            <button onClick={() => handleViewChange('purchased')} className={`py-4 px-2 border-b-2 font-medium text-sm ${currentView === 'purchased' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>
              My Videos
            </button>
          </div>
        </div>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            <div className="flex gap-4">
              <div className="flex-1 relative">
                ...
                <input type="text" placeholder="Search videos..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSearch()} className="w-full pl-10 pr-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
              </div>
              ...
              <button type="button" onClick={handleSearch} disabled={loading} className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50">
          ...
          <>
            ...
            {pagination.pages > 1 && (
              <div className="flex justify-center space-x-2">
                <button onClick={() => setCurrentPage(Math.max(1, currentPage - 1))} disabled={currentPage === 1 || loading} className="px-4 py-2 border rounded-lg hover:bg-gray-50 disabled:opacity-50">Previous</button>
                <span className="px-4 py-2">Page {currentPage} of {pagination.pages}</span>
                <button onClick={() => setCurrentPage(Math.min(pagination.pages, currentPage + 1))} disabled={currentPage === pagination.pages || loading} className="px-4 py-2 border rounded-lg hover:bg-gray-50 disabled:opacity-50">Next</button>
              </div>
            )}
            ...
            {!loading && displayVideos.length === 0 && (
              <div className="text-center py-12">
                ...
                <p className="text-gray-600 text-lg">{currentView === 'purchased' ? 'No purchased videos yet' : 'No videos found'}</p>
                <p className="text-gray-500 text-sm mt-2">{currentView === 'purchased' ? 'Start browsing and purchasing videos to build your library' : 'Try adjusting your search criteria'}</p>
              </div>
            )}
          </>
        )}
