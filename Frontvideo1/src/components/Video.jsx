// Video.jsx — Dark Cinema Theme · Mobile-First · Responsive Navbar
import React, { useState, useEffect, useRef, useCallback } from 'react';
import {
  Play, Search, Upload, ShoppingCart, User, Video,
  Clock, Tag, CheckCircle, AlertCircle,
  QrCode, Loader, X, Home, BookOpen, Flame, ChevronLeft, ChevronRight, SlidersHorizontal, Sparkles
} from 'lucide-react';
...
const styles = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .vs-bnav-center {
    ...
    background: linear-gradient(135deg, var(--accent), var(--accent2));
    display: flex; align-items: center; justify-content: center;
    box-shadow: 0 4px 20px rgba(232,68,90,0.5);
  ...
  @media (max-width: 640px) {
    .vs-page { padding-bottom: var(--bot-nav); }
  }
  ...
  .vs-play-circle {
    ...
    display: flex; align-items: center; justify-content: center;
    box-shadow: 0 4px 24px rgba(232,68,90,0.6);
  ...
  .vs-btn-buy {
    background: linear-gradient(135deg, var(--accent), var(--accent2));
    color: #fff;
    box-shadow: 0 4px 16px rgba(232,68,90,.35);
  }
  ...
  .vs-owned-badge {
    ...
    margin-top: 10px; font-size: 11px; color: var(--success);
    font-weight: 500;
  }

  .vs-admin-badge {
    display: flex; align-items: center; gap: 4px;
    margin-top: 8px; font-size: 10px; color: #60a5fa; font-weight: 600;
  ...
  .vs-pagination {
    display: flex; align-items: center; justify-content: center;
    gap: 10px; margin-top: 36px; padding-bottom: 16px;
  }
  .vs-page-btn {
    display: flex; align-items: center; gap: 5px;
    padding: 8px 18px; border-radius: 10px;
  ...
  .vs-modal-header {
    padding: 24px;
    background: linear-gradient(135deg, rgba(232,68,90,.15), rgba(255,140,66,.1));
    border-bottom: 1px solid var(--border);
    position: relative;
...
const api = {
  ...
  purchaseVideo: async (id) => {
    const res = await fetch(`${API_BASE}/videos/${id}/purchase`, {
      method: 'POST',
  ...
  playVideo: async (id) => {
    const res = await fetch(`${API_BASE}/videos/${id}/play`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${localStorage.getItem('authToken')}`, 'Content-Type': 'application/json' },
      credentials: 'include',
    });
  ...
  getPurchasedVideos: async (params = {}) => {
    const query = new URLSearchParams(params).toString();
    const res = await fetch(`${API_BASE}/videos/purchased/list?${query}`, {
      headers: { Authorization: `Bearer ${localStorage.getItem('authToken')}` },
      credentials: 'include',
    });
    if (!res.ok) throw new Error('Failed to fetch purchased videos');
    return res.json();
  },
  initializeUpload: async (data) => {
    const res = await fetch(`${API_BASE}/videos/upload/initialize`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${localStorage.getItem('authToken')}`, 'Content-Type': 'application/json' },
      credentials: 'include',
  ...
  completeUpload: async (id) => {
    const res = await fetch(`${API_BASE}/videos/upload/${id}/complete`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${localStorage.getItem('authToken')}`, 'Content-Type': 'application/json' },
      credentials: 'include',
    });
  ...
  failUpload: async (id, error) => {
    const res = await fetch(`${API_BASE}/videos/upload/${id}/failed`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${localStorage.getItem('authToken')}`, 'Content-Type': 'application/json' },
      credentials: 'include',
      body: JSON.stringify({ error }),
    });
    if (!res.ok) throw new Error('Failed to record upload failure');
    return res.json();
  },
};
          ...
          {qrImageUrl && paymentStatus === 'checking' && (
            <>
              ...
              <div className="vs-payment-status">
                <Clock size={16} color="#60a5fa" style={{ animation: 'pulse 1.5s ease-in-out infinite' }} />
...
const UploadModal = ({ isOpen, onClose, onUpload }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const up = await fetch(init.uploadUrl, { method: 'PUT', body: file, headers: { 'Content-Type': file.type }, credentials: 'include' });
...
const VideoStreamingApp = () => {
  ...
  const handlePlay = async (video) => {
    ...
    try {
      ...
      setPlayer({ manifestUrl: r.manifestUrl, videoId: video.id || video._id, userIdRef: currentUserIdRef });
